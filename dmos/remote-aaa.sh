...
ssh_config(){
    ...
    "$USERNAME"@"$ip_address" < "config/dmos-aaa.conf"
}
...
for ip_address in 100.127.0.{1..110}; do
    if ping -c 3 -q -W 3 "$ip_address" > /dev/null 2>&1; then
        ssh-keygen -f "$HOME/.ssh/known_hosts" -R "$ip_address" > /dev/null 0>&1
        ...
        if [ "$get_device_aaa_pass_admin" == "$aaa_pass_admin" ] && \
           [ "$get_device_aaa_pass_tacacs" == "$aaa_pass_tacacs" ] || \
           [ "$get_device_aaa_user_luan" == "luan" ]; then
